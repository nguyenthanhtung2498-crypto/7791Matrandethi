...
import { 
  Zap, CheckCircle, FileCode, LayoutGrid, Layers, Info, 
  ShieldCheck, FileDown, Lock, Brain, ChevronRight, Calendar, Trash2,
  Key, FileText, ListOrdered, CheckSquare, MessageSquare, PenTool, BookOpen, Upload, FileUp, AlertTriangle
} from 'lucide-react';
...
const App: React.FC = () => {
  const [state, setState] = useState<AppState>(() => {
    const saved = localStorage.getItem('app_state_7991_v10');
    if (saved) {
      try {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setAuthError(null);
    if (!loginForm.username || !loginForm.apiKey) return setAuthError("Vui lòng điền đủ thông tin.");
    if (!ALLOWED_USERNAMES.includes(loginForm.username.toLowerCase())) return setAuthError("Tài khoản không hợp lệ.");
    ...
    if (result.valid) {
      ...
      setAuthError(`Lỗi API Key: ${result.error}`);
    }
  };

  const handleAnalyze = async () => {
    if (!state.config.filePL1 || !state.config.filePL3) return alert("Vui lòng chọn Phụ lục I và III!");
    setLoading(true);
    try {
  ...
  const exportToWord = (elementId: string) => {
    ...
    // Note: We use basic HTML to Word export. Inline CSS for font.
    const blob = new Blob(['\ufeff', `
      <html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
      <head><meta charset='utf-8'>
      <style>
        body { font-family: 'Times New Roman', serif; color: black; line-height: 1.5; font-size: 12pt; }
        table { border-collapse: collapse; width: 100%; border: 1px solid black; }
        td, th { border: 1px solid black; padding: 5px; font-family: 'Times New Roman', serif; }
    ...
    link.download = `7991_DeThi_Export.doc`;
    link.click();
  };
    ...
    <div className="min-h-screen pb-10 bg-slate-100 text-black font-sans">
      <header className="bg-blue-900 text-white p-5 sticky top-0 z-[100] flex justify-between items-center shadow-lg px-10">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => { localStorage.removeItem('app_state_7991_v10'); window.location.reload(); }} className="text-xs font-bold flex items-center gap-2 hover:bg-red-600 p-2 rounded-lg transition-colors">
          ...
          {state.currentPage === 1 && (
            <div className="max-w-md mx-auto py-10">
              <div className="text-center mb-10">
                ...
                <h2 className="text-3xl font-black uppercase text-blue-950">Đăng nhập</h2>
              </div>
              <form onSubmit={handleLogin} className="space-y-6">
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-4">Người dùng</label>
                  <input className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" placeholder="admin, root..." value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-4">API Key</label>
                  <input type="password" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none" placeholder="Nhập Gemini Key..." value={loginForm.apiKey} onChange={e => setLoginForm({...loginForm, apiKey: e.target.value})} />
                </div>
                {authError && <div className="p-4 bg-red-50 text-red-600 font-bold text-xs rounded-2xl">{authError}</div>}
                <button type="submit" disabled={validating} className="w-full bg-blue-800 text-white py-6 rounded-[2.5rem] font-black uppercase shadow-xl">VÀO ỨNG DỤNG</button>
              </form>
            </div>
          )}
          ...
          {state.currentPage === 2 && (
             <div className="grid lg:grid-cols-2 gap-12">
               <div className="space-y-8">
                 <h3 className="text-2xl font-black uppercase border-l-8 border-blue-600 pl-4">CẤU HÌNH</h3>
                 ...
                 <div className="space-y-4">
                    <div className="p-6 border-2 border-dashed rounded-3xl bg-slate-50">
                      <p className="text-[10px] font-black uppercase mb-3">Phụ lục I (Đặc tả mẫu)</p>
                      <input type="file" onChange={e => setState(p => ({...p, config: {...p.config, filePL1: e.target.files ? e.target.files[0] : null}}))} />
                    </div>
                    <div className="p-6 border-2 border-dashed rounded-3xl bg-slate-50">
                      <p className="text-[10px] font-black uppercase mb-3">Phụ lục III (Kế hoạch GD)</p>
                      <input type="file" onChange={e => setState(p => ({...p, config: {...p.config, filePL3: e.target.files ? e.target.files[0] : null}}))} />
                    </div>
                 </div>
                 <button onClick={handleAnalyze} disabled={!state.config.filePL1 || !state.config.filePL3} className="w-full bg-blue-800 text-white py-6 rounded-[2rem] font-black uppercase shadow-xl">TIẾP TỤC</button>
               </div>
               <div className="bg-slate-50 p-8 rounded-[3rem] border-2 border-slate-100 flex flex-col">
                  <h3 className="font-black uppercase mb-6 text-blue-700 tracking-tighter flex items-center gap-3"><Layers /> Bài học phát hiện</h3>
                  ...
                  {state.selectedLessons.length > 0 && <button onClick={() => setState(p => ({...p, currentPage: 3}))} className="mt-8 bg-blue-900 text-white py-6 rounded-[2.5rem] font-black uppercase tracking-widest">TIẾP THEO <ChevronRight className="inline ml-2" /></button>}
               </div>
             </div>
          )}
          ...
          {state.currentPage === 3 && (
            <div className="max-w-5xl mx-auto py-10">
              <div className="text-center mb-12">
                <h3 className="text-4xl font-black uppercase text-blue-950">Khung đề chuẩn (Tổng 10đ)</h3>
              </div>
              ...
              <div className="flex justify-center gap-6 mt-20">
                ...
                <button onClick={handleGenerateMatrix} className="px-28 py-6 bg-blue-800 text-white rounded-full font-black uppercase shadow-2xl">LẬP MA TRẬN TỰ ĐỘNG <Zap className="inline" /></button>
              </div>
            </div>
          )}
          ...
          {state.currentPage === 4 && state.matrix && (
            <div className="space-y-8">
              <div className="flex justify-between items-end border-b pb-8 text-black">
                <div>
                  <h3 className="text-4xl font-black uppercase tracking-tighter">Ma trận (Phân bổ 20% GK nếu là CK)</h3>
                </div>
                <button onClick={() => setState(p => ({...p, currentPage: 5}))} className="bg-blue-800 text-white px-16 py-6 rounded-full font-black uppercase shadow-xl hover:bg-black transition-all">BẢN ĐẶC TẢ <ChevronRight className="inline ml-2" /></button>
              </div>
             ...
             <div className="space-y-8">
                <div className="flex justify-between items-center bg-emerald-50/50 p-8 rounded-[3rem] border-2 border-emerald-100">
                  <div className="max-w-xl text-black">
                    <h3 className="text-3xl font-black uppercase">Đặc tả Yêu cầu cần đạt</h3>
                    <div className="mt-4 flex flex-col gap-3">
                      <p className="text-[11px] font-black text-emerald-700 uppercase">Nạp SGK (PDF) để AI bám sát dữ liệu:</p>
                  ...
                  <button onClick={handleCreateFinalExam} className="bg-emerald-700 text-white px-16 py-8 rounded-full font-black uppercase shadow-2xl flex items-center gap-4 hover:bg-black transition-all">SOẠN ĐỀ THI <Zap /></button>
                </div>
            ...
            <div className="space-y-10">
               <div className="flex justify-between items-center bg-blue-50 p-10 rounded-[4rem] border-2 border-blue-100 shadow-inner">
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-blue-500 uppercase mt-2 italic flex items-center gap-2"><CheckCircle className="w-4 h-4" /> TIMES NEW ROMAN • LaTeX TOÁN/HÓA</p>
                  </div>
                  <div className="flex gap-4">
                    <button onClick={() => exportToWord('exam-doc')} className="bg-emerald-600 text-white px-12 py-5 rounded-2xl font-black uppercase flex items-center gap-3 shadow-xl"><FileDown className="w-6 h-6" /> Xuất Word</button>
               ...
               <div id="exam-doc" className="bg-white border p-20 rounded-[4rem] shadow-sm mx-auto max-w-5xl times-new-roman text-[12pt] leading-relaxed min-h-[900px] text-black">
                  ...
                  <div className="space-y-12">
                    ...
                    <section><p className="font-bold uppercase underline mb-6">PHẦN III. TRẢ LỜI NGẮN (2,0 ĐIỂM)</p>
          ...
          {state.currentPage === 7 && state.gradingGuide && (
            <div className="space-y-10">
               <div className="flex justify-between items-center bg-indigo-50 p-10 rounded-[3rem] border-2 border-indigo-100 shadow-inner">
                  <h3 className="text-3xl font-black text-indigo-900 uppercase">Hướng dẫn chấm</h3>
               ...
               <div id="guide-doc" className="bg-white border p-20 rounded-[4rem] shadow-sm mx-auto max-w-5xl times-new-roman text-[12pt] min-h-[900px] text-black">
                  <h2 className="text-center font-bold text-2xl uppercase mb-12 border-b-4 pb-6 border-slate-900 text-black">ĐÁP ÁN VÀ BIỂU ĐIỂM</h2>
                  <div className="mb-12"><p className="font-bold uppercase underline mb-6">PHẦN I. MCQ (3,0đ)</p>
                    <table className="word-export-table border-collapse w-full text-center">
                  ...
                  <div className="mb-12"><p className="font-bold uppercase underline mb-8">PHẦN III & IV. TỰ LUẬN</p>
                    <div className="space-y-10">
                      {state.gradingGuide.shortAnswers.map((item, i) => (
                        <div key={`s-${i}`} className="p-8 bg-slate-50 rounded-3xl border-2 border-slate-100"><p className="font-black mb-4 uppercase">Câu {item.questionNum} (Ngắn):</p><div dangerouslySetInnerHTML={{__html: item.answer}}></div></div>
                      ))}
                      {state.gradingGuide.essayAnswers.map((item, i) => (
                        <div key={`e-${i}`} className="p-8 bg-slate-50 rounded-3xl border-2 border-slate-100"><p className="font-black mb-4 uppercase">Câu {item.questionNum} (Tự luận):</p><div dangerouslySetInnerHTML={{__html: item.guide}}></div></div>
                      ))}
                    </div>
                  </div>
               </div>
      ...
      {loading && (
        <div className="fixed inset-0 bg-blue-950/95 backdrop-blur-xl flex flex-col items-center justify-center z-[5000]">
          ...
          <h4 className="text-white font-black text-3xl uppercase tracking-widest animate-pulse text-center px-10">AI ĐANG TÍNH TOÁN PHÂN BỔ ĐIỂM</h4>
        </div>
      )}
    </div>
  );
